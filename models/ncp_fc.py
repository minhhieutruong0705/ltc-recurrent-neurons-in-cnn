...
class NCP_FC(nn.Module):
    def __init__(self,
        ...
        ltc_cell = LTCCell(wiring=wiring, in_features=sensory_neurons)
        self.ltc_seq = RNNSequence(ltc_cell)

        self.fc = nn.Linear(motor_neurons * seq_len, classes)
...
if __name__ == '__main__':
    ...
    model = NCP_FC(seq_len=32, sensory_neurons=64, classes=2)
    y = model(x)
    assert y.size() == (8, 2)
