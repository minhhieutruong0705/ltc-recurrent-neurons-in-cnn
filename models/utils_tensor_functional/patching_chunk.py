...
if __name__ == '__main__':
    ...
    chunks = chunks.split(split_size=1, dim=0)  # split patches
    chunks = torch.concat(chunks, dim=3)  # stack patches horizontally
    chunks = chunks.squeeze(dim=0)  # (P, C, H, W) -> (C, H, W)
    __show_tensor_img__(chunks)