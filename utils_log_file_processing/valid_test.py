...
def track_training(data_dict, include_test=True, **plt_kwarg):
    x = range(len(data_dict["[TRAIN]"]["Loss"]))  # number of epochs

    # plot train & eval loss
    plot_fig(fig_name="train-val_loss", x=x, ys=[data_dict["[TRAIN]"]["Loss"], data_dict["[EVAL]"]["Loss"]],
             y_names=["train loss", "validation loss"], bound_value=0, **plt_kwarg)

    # plot train & eval accuracy & dice
    plot_fig(fig_name="train-val_accuracy-dice", x=x,
    ...
    # plot train
    plot_fig(fig_name="train-metrics", x=x,
    ...
    # plot eval
    plot_fig(fig_name="validation-metrics", x=x,
    ...
    if include_test:  # after training is finished
        # plot test
        plot_fig(fig_name="test-metrics", x=x,
...
def cal_mean_std(data_dict, metrics, start_i, end_i, save_file_path):
