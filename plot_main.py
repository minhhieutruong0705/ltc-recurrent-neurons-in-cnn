...
def __plot_fig__(fig_name, x, ys, y_names, bound_value, save_dir=None):
    # init figure
...
def __show_fig__(data_dict, save_dir=None, is_track_early_stop=False):
    ...
    __plot_fig__(fig_name="train-val_loss", x=x, ys=[data_dict["[TRAIN]"]["Loss"], data_dict["[EVAL]"]["Loss"]],
                 y_names=["train loss", "validation loss"], bound_value=0, save_dir=save_dir)
    ...
    __plot_fig__(fig_name="validation-metrics", x=x,
                 ...
                 y_names=["validation accuracy", "validation f1", "validation dice"], bound_value=100, save_dir=save_dir)
...
def process_log(log_file, save_dir, is_track_early_stop=False):
    data_dict = __parse_data__(log_file)
    __show_fig__(data_dict, save_dir=save_dir, is_track_early_stop=is_track_early_stop)
    if not is_track_early_stop:
        __cal_stats__(
            data_dict=data_dict,
            metrics=["[EVAL]:Acc", "[EVAL]:F1", "[EVAL]:Dice"],
            start_i=149,
            end_i=-1,
            save_dir=save_dir
        )
